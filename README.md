...
실행 결과로 차트 이미지 3개가 저장됩니다:
- `chart_revenue.png` — 강의별 매출 & 구매 횟수
- `chart_hourly.png` — 시간대별 이벤트 추이
- `chart_completion.png` — 강의별 완강률
...
`analyze.py`에서 matplotlib으로 차트를 이미지 파일로 저장하고, Metabase를 docker-compose에 추가해 대시보드도 함께 구성했습니다. 두 방법을 병행함으로써 코드 기반 시각화와 BI 도구 기반 시각화를 모두 경험할 수 있었습니다.
