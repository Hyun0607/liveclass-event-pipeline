...
이벤트 생성기 앱을 클러스터 환경에서 안정적으로 운영하기 위한 쿠버네티스(Kubernetes) 매니페스트 설계안입니다. 
([`k8s/deployment.yaml`](./k8s/deployment.yaml), [`k8s/service.yaml`](./k8s/service.yaml))

### 1. 선택한 Kubernetes 리소스의 역할
- **Deployment**: 이벤트 생성기(`event_generator.py`)가 담긴 도커 이미지를 쿠버네티스 상에서 몇 개를 띄울지, 어떤 자원을 사용할지 정의하고 파드(Pod)의 상태를 관리하는 역할을 합니다.
- **Service**: 배포된 이벤트 생성기 파드들이 외부나 내부의 다른 서비스(MySQL 등)와 안정적으로 통신할 수 있도록 고정된 네트워크 통로(IP 및 포트)를 열어주는 역할을 합니다.
