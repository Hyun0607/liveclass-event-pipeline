...
비율은 실제 교육 플랫폼의 사용자 행동 흐름을 반영해 설계했습니다. 강의를 재생(`lecture_play`)하는 행동이 가장 빈번하게 발생하므로 40%로 가장 높게 설정했고, 끝까지 시청(`lecture_complete`)하는 비율은 그보다 낮은 25%로 설정해 현실적인 완강률(약 62.5%)이 자연스럽게 나오도록 했습니다. 구매(`course_purchase`)는 핵심 전환 행동이지만 재생보다는 적게 발생하므로 20%로 설정했습니다. 리뷰(`review_submit`)는 구매 후 일부만 작성하므로 10%, 에러(`error`)는 정상적인 서비스에서 드물게 발생해야 하므로 5%로 가장 낮게 설정했습니다.
...
`event_id`(UUID)를 PK이자 FK로 사용해 두 테이블을 연결하며, 자주 쓰는 `event_type`, `user_id`, `timestamp` 컬럼에 인덱스를 걸어 집계 쿼리 성능을 높였습니다.
...
> 완강률을 어떻게 계산해야 할지 처음에는 막막했습니다. `lecture_play`와 `lecture_complete`는 서로 직접 연결된 키가 없기 때문에 단순 JOIN이 불가능했습니다. 결국 두 테이블을 각각 `course_id` 기준으로 집계한 뒤 LEFT JOIN하는 방식으로 해결했고, 구매 이력이 없는 강의의 course_title을 가져오기 위해 `event_course_purchase`를 한 번 더 JOIN했습니다.
...
Metabase는 MySQL과 직접 연결되어 별도 코드 없이 SQL 결과를 바로 시각화할 수 있어 선택했습니다. 대시보드 자동 새로고침 기능을 통해 이벤트 데이터가 쌓일 때마다 실시간으로 현황을 모니터링할 수 있다는 점도 장점입니다.

완강률 차트를 구성할 때 고민이 있었습니다. 재생 수(`play_count`)와 완강 수(`complete_count`)는 절대적인 수치이고 완강률(`completion_rate_pct`)은 비율이라 단일 축으로 표현하면 수치 차이로 인해 완강률이 눈에 잘 띄지 않는 문제가 있었습니다. 이를 해결하기 위해 재생/완강 수는 Bar chart로, 완강률은 보조 축을 활용한 Line chart로 이중 표현해 두 지표가 모두 명확하게 드러나도록 구성했습니다.
